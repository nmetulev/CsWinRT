...
namespace UnitTest
{
    ...
    public class UnitTestCSharp
    {
        ...
        [DataRow(typeof(WeakReference<TestComponent.Class>), "System.WeakReference`1[[TestComponent.Class, WinRT.Projection, Version=0.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35]], System.Private.CoreLib, Version=10.0.0.0, Culture=neutral, PublicKeyToken=7cec85d7bea7798e", "Custom")]
        ...
        async Task TestStorageFolderAsync()
        {
            ...
            var handle = folder.CreateSafeFileHandle("TestComponentCSharp.dll", FileMode.Open, FileAccess.Read);
            Assert.IsNotNull(handle);
        }
        ...
        [TestMethod]
        public void TestWriteBufferPinnedArrayBuffer()
        {
            async Task TestAsync()
            {
                var random = new Random(42);
                ...
                // WindowsRuntimeBuffer.Create() creates a WindowsRuntimePinnedArrayBuffer,
                // which exercises a separate CCW code path than AsBuffer() (which creates
                // a WindowsRuntimeExternalArrayBuffer). This test verifies that the pinned
                // array buffer CCW is correctly used when writing to a native WinRT stream.
                using var stream = new InMemoryRandomAccessStream();
                IBuffer buffer = WindowsRuntimeBuffer.Create(data);
                await stream.WriteAsync(buffer);
        ...
        */
        ...
        public void TestOverridable()
        {
            ...
            Assert.AreEqual(42, obj.CallOverridablePropertyGetter());
            obj.CallOverridablePropertySetter(99);
            Assert.AreEqual(99, obj.CallOverridablePropertyGetter());

            // Test overridable method round-trip through native overrides interface
            Assert.IsFalse(obj.MethodWasCalled);
            obj.CallOverridableMethod();
            Assert.IsTrue(obj.MethodWasCalled);
        }
