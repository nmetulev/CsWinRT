    ...
    public async Task IndexerReads_Warn()
    {
        const string source = """
            ...
            public class Sample
            {
                public void Fill(Span<int> span)
                {
                    int value = {|CSWINRT2018:span[0]|};
                    Value({|CSWINRT2018:span[1]|});
                    ref readonly int readOnlyReference = ref {|CSWINRT2018:span[2]|};
                    In(in {|CSWINRT2018:span[3]|});
                    {|CSWINRT2018:span[4]|}++;
                    {|CSWINRT2018:span[5]|} += 1;
                }
    ...
    public async Task ReadOnlySpanConversions_Warn()
    {
        const string source = """
            ...
            public class Sample
            {
                public void Fill(Span<int> span)
                {
                    ReadOnlySpan<int> implicitConversion = {|CSWINRT2018:span|};
                    ReadOnlySpan<int> explicitConversion = {|CSWINRT2018:(ReadOnlySpan<int>)span|};

                    Read({|CSWINRT2018:span|});
                }
    ...
    public async Task ForEachReads_Warn()
    {
        const string source = """
            ...
            public class Sample
            {
                public void Fill(Span<int> span)
                {
                    foreach (int x in {|CSWINRT2018:span|})
                    {
                    }

                    foreach (ref readonly int y in {|CSWINRT2018:span|})
                    {
                    }
                }
            }
